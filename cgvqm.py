...
class CGVQM(resnet18.VideoResNet):
    def init_weights(self, weights_file=None, num_layers=6):
        ...
        self.register_parameter(
            "feature_weights",
            nn.Parameter(torch.ones(1, sum(self.chns), 1, 1, 1).to(device)),
        )
        self.register_parameter("alpha", nn.Parameter(torch.tensor(1.0).to(device)))
    ...
    def feature_diff(self, x: Tensor, y: Tensor, cache_path=None):
        def fdiff(a, b, w, normalize_channel=True):
            ...
            emap = torch.nn.functional.interpolate(
                emap, size=tuple(x.shape[2:]), mode="trilinear"
            )
            cache = (
                (a - b).pow(2).mean([2, 3, 4], keepdim=True)
        ...
        cache = []
        diff = 0
        emap = torch.zeros(x.shape[0], 1, x.shape[2], x.shape[3], x.shape[4]).to(
            self.get_device()
        )
...
def run_cgvqm(
    ...
    if cgvqm_type == CGVQM_TYPE.CGVQM_2:
        model.init_weights(
            os.path.join(dir_path, "weights", "cgvqm-2.pickle"), num_layers=3
        )
    elif cgvqm_type == CGVQM_TYPE.CGVQM_5:
        model.init_weights(
            os.path.join(dir_path, "weights", "cgvqm-5.pickle"), num_layers=6
        )
    else:
    ...
    if D.shape[3] % patch_scale != 0 or D.shape[4] % patch_scale != 0:
        print(
            f"WARNING: Spatial resolution not divisible by {patch_scale}. Error map resolution might not match input videos"
        )
    ...
    D = torch.nn.functional.pad(
        ...
        (
            0,
            (ps[1] - D.shape[4] % ps[1]) % ps[1],
            0,
            (ps[0] - D.shape[3] % ps[0]) % ps[0],
            0,
            (clip_size - D.shape[2] % clip_size) % clip_size,
        ),
        mode="replicate",
    )
    R = torch.nn.functional.pad(
        ...
        (
            0,
            (ps[1] - R.shape[4] % ps[1]) % ps[1],
            0,
            (ps[0] - R.shape[3] % ps[0]) % ps[0],
            0,
            (clip_size - R.shape[2] % clip_size) % clip_size,
        ),
        mode="replicate",
    )
        ...
        for h in range(0, D.shape[3], ps[0]):
            for w in range(0, D.shape[4], ps[1]):
                Cd = D[
                    :,
                    :,
                    i : min(i + clip_size, D.shape[2]),
                    h : h + min(ps[0], D.shape[3]),
                    w : w + min(ps[1], D.shape[4]),
                ].to(device)
                Cr = R[
                    :,
                    :,
...
def demo_cgvqm():
    ...
    # Configuration
    dist_vid_path = os.path.join(
        dir_path, "media/Dock_dist.mp4"
    )  # Path to distorted video
    ref_vid_path = os.path.join(
        dir_path, "media/Dock_ref.mp4"
    )  # Path to reference video
    errmap_path = os.path.join(
        dir_path, "media/Dock_dist_emap.mp4"
    )  # Path to save CGVQM predicted error map
    ...
    print(
        f"Quality of Dock_dist is {q.item():.2f}/100 ({qlabels[int(np.round(q.item() / 25))]}). Quality map written to media/Dock_dist_emap.mp4"
    )
